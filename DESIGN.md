...
- Still open, in order of measured size:
  ...
  - Code Start auto-resumes the newest conversation whatever its size, so opening the app can inherit a very large prefix and pay for it on the first message. The 45% mark now caps how large that prefix gets in the first place, which is why this is a follow-up rather than part of this change
